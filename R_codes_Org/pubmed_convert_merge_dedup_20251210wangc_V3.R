rm(list = ls())
setwd("D:\\idiv\\Literature\\Pubmed")

convert_dir <- "convert2"   # Folder storing the TXT files
...
for(in_file in txt_files){
  ...
  # Extract keywords: MH (MeSH terms) + OT (Other Terms)#
  get_keywords <- function(rec){
    mh_hits <- rec[grepl("^MH  - ", rec)]   # MeSH terms
    ot_hits <- rec[grepl("^OT  - ", rec)]   # Other Terms
    
    # Remove prefixes
    mh <- sub("^MH  - ", "", mh_hits)
    ot <- sub("^OT  - ", "", ot_hits)
    
    # Combine, remove empty, unique
    kws <- unique(c(mh, ot))
    kws <- kws[kws != ""]
    
    if(length(kws) == 0) return(character(0))
  ...
  record_to_ris <- function(rec){
    ...
    ris <- c(
      ...
      if(!is.na(pmid))   paste0("ID  - ", pmid)  else NA,  # PMID stored in ID
      if(!is.na(title))  paste0("TI  - ", title) else NA,  # Title
      aus,                                                  # Authors (multi-line)
      if(!is.na(year))   paste0("PY  - ", year)  else NA,  # Year
      if(!is.na(journal))paste0("JO  - ", journal) else NA,# Journal
      if(!is.na(vol))    paste0("VL  - ", vol)   else NA,  # Volume
      if(!is.na(issue))  paste0("IS  - ", issue) else NA,  # Issue
      if(!is.na(sp))     paste0("SP  - ", sp)    else NA,  # Start page
      if(!is.na(ep))     paste0("EP  - ", ep)    else NA,  # End page
      if(length(kws)>0)  kws                       else NA, # Keywords (multi-line KW)
      if(!is.na(ab))     paste0("AB  - ", ab)    else NA,  # Abstract
      if(!is.na(doi))    paste0("DO  - ", doi)   else NA,  # DOI
      "ER  - ",                                             # End of record
      ""                                                    # Blank line
    )
...
convert_dir <- "D:\\idiv\\Literature\\Pubmed\\convert2"
...
#----------------------------------------
dedup_file <- "D:\\idiv\\Literature\\Pubmed\\convert2\\pubmed_merged_dedup.ris"
